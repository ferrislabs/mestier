...
### Sourcing one key from a different Secret (`api.secret.valueFrom`)

Sometimes only *some* of the sensitive values live in a Secret you don't
control, and that Secret uses its own key names — e.g. a Secret generated by
an operator such as CloudNativePG. `api.secret.valueFrom` lets you override
individual keys without giving up the chart-managed Secret for the rest:
each entry replaces that env var's source with a `secretKeyRef` to the
Secret/key you name, and the chart stops writing that key into its own
Secret (no empty duplicate). Keys without an entry keep the default
behavior (chart-managed Secret, same key name as `api.secret.<KEY>`).

```yaml
api:
  secret:
    AUTH_CLIENT_SECRET: <...>            # still chart-managed
    valueFrom:
      DATABASE_PASSWORD:
        secretName: mestier-db-app       # Secret generated by CloudNativePG
        key: password                    # not DATABASE_PASSWORD
