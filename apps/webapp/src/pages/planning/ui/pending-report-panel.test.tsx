...
describe('PendingReportPanel', () => {
	it('renders nothing when there is no pending report', async () => {
	...
	it('shows what was planned, what was reported, and who said so', async () => {
		await renderWithPermissions(<PendingReportPanel {...baseProps()} />)
	...
	it('clicking Appliquer arms the report and shows the confirmation sentence', async () => {
		...
		await renderWithPermissions(<PendingReportPanel {...props} />)
	...
	it('the armed report shows the confirmation sentence instead of the action buttons', async () => {
		await renderWithPermissions(
			<PendingReportPanel {...baseProps()} applyingReportId="report-1" />,
		)
	...
	it('shows the resolve error near the report it belongs to', async () => {
		await renderWithPermissions(
			<PendingReportPanel
				{...baseProps()}
