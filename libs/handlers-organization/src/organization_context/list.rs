use auth::Identity;
use axum::{
	Extension,
	extract::{Query, State},
};
use handlers::{
	ApiError, AppState, DataEnvelope, Page, PaginationMetadata, PaginationParams, Response,
};

use crate::{
	organization_context::response::OrganizationContextResponse,
	paths::OrganizationContextsPath, require_org_membership,
};

#[utoipa::path(
	get,
	path = "/api/v1/organizations/{organization_id}/organization-contexts",
	operation_id = "listOrganizationContexts",
	tag = super::super::TAG,
	params(
		("organization_id" = mestier_core::OrganizationId, Path, description = "Organization identifier"),
		PaginationParams,
	),
	responses(
		(status = 200, description = "Paginated list of organization contexts", body = inline(DataEnvelope<Vec<OrganizationContextResponse>>)),
		(status = 401, description = "Unauthorized"),
		(status = 403, description = "Forbidden"),
	),
	security(("bearer_auth" = []))
)]
pub async fn handler(
	path: OrganizationContextsPath,
	State(state): State<AppState>,
	Extension(identity): Extension<Identity>,
	Query(pagination): Query<PaginationParams>,
) -> Result<Response<OrganizationContextResponse>, ApiError> {
	require_org_membership(&state, &identity, path.organization_id).await?;

	let per_page = pagination.per_page();
	let page = pagination.page();
	let offset = pagination.offset();
	let (organization_contexts, total) = state
		.usecase
		.list_organization_contexts(path.organization_id, per_page, offset)
		.await?;
	let items: Vec<OrganizationContextResponse> = organization_contexts
		.into_iter()
		.map(OrganizationContextResponse::from)
		.collect();
	let is_empty = items.is_empty();
	let meta = PaginationMetadata::new(per_page, page, Some(total), is_empty);

	Ok(Response::Paginated(Page::new(items, meta)))
}
