...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Paginated list of service rates", body = inline(DataEnvelope<Vec<ServiceRateResponse>>)),
...
pub async fn handler(
    ...
    let (service_rates, total) = state
        .usecase
        .list_service_rates(path.organization_id, per_page, offset)
        .await?;
    let items: Vec<ServiceRateResponse> = service_rates
        .into_iter()
        .map(ServiceRateResponse::from)
        .collect();
