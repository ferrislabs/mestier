...
use crate::{
    absence::{AbsenceResponse, AbsencesPath},
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Paginated list of absences", body = inline(DataEnvelope<Vec<AbsenceResponse>>)),
...
pub async fn handler(
    ...
    let (absences, total) = state
        .usecase
        .list_absences(path.organization_id, per_page, offset)
        .await?;
    let items: Vec<AbsenceResponse> = absences.into_iter().map(AbsenceResponse::from).collect();
