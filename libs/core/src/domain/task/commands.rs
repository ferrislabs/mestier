...
use crate::{
    ...
    domain::task::{AssigneeRef, BoardRank, TaskId, TaskStatus},
...
pub struct PatchTaskCommand {
    ...
    /// Where the card now sits inside its column, following the same double
    /// option as the window and `parent_task_id` above: `None` leaves the
    /// rank alone, `Some(None)` clears it (the card falls back to the end of
    /// the column, where every never-dragged task already is), and
    /// `Some(Some(rank))` sets it.
    ///
    /// The rank itself is generated by [`BoardRank::between`] from the two
    /// cards the drop landed between — by the caller, which is the only layer
    /// that knows what the board looked like when the user let go. Carrying
    /// the rank rather than the two neighbour ids keeps this command a plain
    /// description of the new state, and keeps `TaskService::patch_task` from
    /// having to re-read a column to interpret it.
    ///
