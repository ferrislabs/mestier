...
//!
//! The point of this test is that the auth middleware runs for real, so the
//! token has to survive the whole chain: `decode_header`, a JWKS fetch over
//! HTTP, an RS256 signature check against the published modulus, and the
//! expiry check. Handing the middleware a pre-built `Identity` would skip all
//! of it, which is the coverage the test exists to buy.
//!
//! The key pair is the one already committed for `libs/auth`'s own tests. It
//! signs nothing but test tokens.
